import numpy as np
from params_bit import N_phi, N_Z, R, L
from reynolds_solver.utils import create_H_with_ellipsoidal_depressions

# Сетка: phi ∈ [0, 2π), Z ∈ [-1, 1]
# endpoint=False для phi — корректно для периодической координаты
phi_1D = np.linspace(0, 2 * np.pi, N_phi, endpoint=False)
Z_1D   = np.linspace(-1, 1, N_Z)
d_phi  = phi_1D[1] - phi_1D[0]
d_Z    = Z_1D[1] - Z_1D[0]

# Meshgrid: shape (N_Z, N_phi) — совпадает с конвенцией солвера
...
def H_smooth(epsilon):
    """Гладкий зазор: H = 1 + ε·cos(φ). Shape: (N_Z, N_phi)."""
    return 1.0 + epsilon * np.cos(Phi_mesh)


def H_textured(epsilon, cfg: dict):
    """Зазор с эллипсоидальными углублениями. Shape: (N_Z, N_phi)."""
    H0 = H_smooth(epsilon)
