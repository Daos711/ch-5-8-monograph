...
if PLOT_ONLY:
    ...
    results_common = cache["results_common"]
    gains_common   = cache["gains_common"]
    sweep_B        = cache["sweep_B"]
    F_ext          = cache["F_ext"]
    U_eq           = cache["U_eq"]
    F_max_smooth   = cache["F_max_smooth"]
    F_max_best_val = cache["F_max_best_val"]
    F_ext_B_kN     = cache["F_ext_B_kN"]
    eps_scan       = cache["eps_scan"]
    F_scan_all     = cache["F_scan_all"]
    ...
    # ── 7a. Коэффициенты улучшения при F_common — данные ─────────────────
    ...
    # ── Сохранение кэша ─────────────────────────────────────────────────
    cache = dict(
        results=results, results_common=results_common,
        gains_common=gains_common, sweep_B=sweep_B,
        F_ext=F_ext, U_eq=U_eq,
        F_max_smooth=F_max_smooth, F_max_best_val=F_max_best_val,
        F_ext_B_kN=F_ext_B_kN,
        eps_scan=eps_scan, F_scan_all=F_scan_all,
    )
...
# ═════════════════════════════════════════════════════════════════════════════
#  ГРАФИКИ  (всегда выполняется — и при полном расчёте, и при --plot-only)
# ═════════════════════════════════════════════════════════════════════════════
...
for cfg_name, F_scan in F_scan_all.items():
